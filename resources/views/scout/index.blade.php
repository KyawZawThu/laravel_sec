        ...
        <tbody>
          ...
          @endphp
              ...
              <tr>
                ...
                <td>{{$row->user->name}}</td>
                <td>
                  <a href="{{ route('scout.show', $row->id) }}" class="btn btn-info"><i class="fas fa-info-circle fa-pulse"></i></a>
                </td>
              </tr>
               @endforeach
        </tbody>
       ...
       <table id="example1" class="table table-bordered table-striped">
        ...
        <tbody>
           @php 
            $i=1;
          @endphp
              @foreach($confirmed_scouts as $row)
              <tr>
                <td>{{$i++}}</td>
                <td>{{$row->scoutdate}}</td>
                <td>{{$row->notes}}</td>
                ...
                <td>Completed</td>
                <td>
                  <a href="{{route('scout.show',$row->id)}}" class="fas fa-info-circle"></a>                
                </td>
        </tbody>
        @endforeach
      </table>
